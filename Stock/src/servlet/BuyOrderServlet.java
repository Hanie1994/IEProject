...
public class BuyOrderServlet{
    public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(type==null || type.equals("")){
            hasError = true ;
            errMessage.add("type can not be empty");
        }
        ...
        if(!hasError){
            try {
                message = Core.getInstance().buyOrder(id, inst, price , quantity , type);
            } catch (InvalidOrderTypeException | OrderIsDeclinedException | InvalidSymbolIdException | MismatchedParametersException | OrderIsQueuedException | UnknownUserIdException | NotEnoughCreditException | OrderIsLimitedException e) {
                e.printStackTrace();
